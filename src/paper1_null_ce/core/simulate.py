...
from paper1_null_ce.core.summarize import DEFINITION_COLUMNS, headline_rates, summarize_study_level, summarize_window_results
...
def run_pipeline(config: dict[str, Any], mode: str = "smoke") -> dict[str, Any]:
    ...
    progress.update_stage("study_level_monte_carlo", message="simulating n=30 study-level Monte Carlo")
    ...
    summary = pd.concat(
        [summarize_window_results(window_results), summarize_study_level(study_level)],
        ignore_index=True,
        sort=False,
    )
...
def _simulate_one_participant_prepared(
    ...
    daily, shift = merge_independent_diaries(
        seizure.daily,
        hormone.daily,
        seed=stable_seed(master_seed, participant_id, "circular_shift"),
        shift_target=str(config.get("circular_shift_target", "seizure")),
    )
    phase_mode = str(config.get("phase_mode", "strict_herzog"))
    if phase_mode == "modified_short_cycle" and cohort != "population":
        phase_mode = "strict_herzog"
    daily = add_phase_labels(daily, mode=phase_mode)
    true_coupling = {
    ...
    if coupling_applied:
        daily = _apply_true_coupling(
            daily,
            seed=stable_seed(master_seed, participant_id, "true_coupling"),
    ...
    participant_summary.update(
        {
            "seizure_count_total": int(daily["seizure_count"].sum()),
            "seizure_days_total": int(daily["seizure_day"].sum()),
            "seizure_days_per_month": float(daily["seizure_day"].sum() / (len(daily) / days_per_month)),
            "seizures_per_month": float(daily["seizure_count"].sum() / (len(daily) / days_per_month)),
    ...
    alpha = participant_alpha_from_full_diary(daily, fallback=float(config.get("regression", {}).get("fallback_alpha", 1.0)))
    ...
    window_results = [
        classify_window(
            ...
            config,
            alpha,
            include_regression=bool(config.get("regression", {}).get("enabled", True)),
        )
...
def classify_window(
    ...
    base["phase_mode"] = str(config.get("phase_mode", "strict_herzog"))
    ...
    if spec.window_type == "cycle" and str(spec.window_value) == "3":
    ...
    c = classify_reproducibility(
        ...
        cohort,
        thresholds,
        min_complete_cycles=int(config.get("reproducibility", {}).get("primary_min_complete_cycles", 6)),
    )
    c12 = classify_reproducibility(
        window_df,
        cohort,
        thresholds,
        min_complete_cycles=int(config.get("reproducibility", {}).get("sensitivity_min_complete_cycles", 12)),
    )
    if include_regression:
        ...
        d_window_alpha = {
            ...
            "label_D_window_alpha_C2": None,
            "label_D_window_alpha_C3": None,
            "d_window_alpha_reason": "regression_disabled",
        }
...
def run_study_level_mc(study_pool: pd.DataFrame, config: dict[str, Any], mode: str) -> pd.DataFrame:
    ...
    n_per_study = int(config["study_level"]["n_participants"])
    master_seed = int(config["master_seed"])
    rng = np.random.default_rng(stable_seed(master_seed, mode, "study_level_mc"))
    definitions = list(DEFINITION_COLUMNS.items())
    rows: list[dict[str, Any]] = []
    pool_by_cohort = {cohort: g.copy() for cohort, g in study_pool.groupby("cohort", sort=False)}
    for cohort, cohort_pool in pool_by_cohort.items():
        ...
        for study_id in range(n_studies):
            ...
            for definition, col in definitions:
                ...
                rows.append(
                    {
                        "cohort": cohort,
                        "phase_mode": str(study_rows["phase_mode"].iloc[0]) if "phase_mode" in study_rows else str(config.get("phase_mode", "strict_herzog")),
                        "study_id": study_id,
...
def _write_outputs(
    ...
    summary.to_csv(paths[3], index=False)
    audit_daily.to_parquet(paths[4], index=False)
    return paths
